...
import cz.cvut.kbss.owl2query.complexversion.modelold.QueryParameters;
...
public class SparqlDLResultSet implements ResultSetRewindable {
	...
	public SparqlDLResultSet(QueryResult answers, Model model, Binding parent,
		...
		resultVars = new ArrayList<Term>();
		resultVarsString = new ArrayList<String>();

		for( final Term var : queryResult.getResultVars() ) {
			resultVars.add( var );
	...
	private String getVarName( Term term ) {
	       if( ATermUtils.isVar( term ) )
	           return ((Term)term.getArgument(0)).getName();
	...
	public Binding nextBinding() {
		...
		if( resultVars.size() == 0 ) {
			...
			for( Iterator<Entry<Term, Term>> iter = parameters.entrySet().iterator(); iter
					.hasNext(); ) {
				Entry<Term, Term> entry = iter.next();

...
import cz.cvut.kbss.owl2query.owlapi.exceptions.InternalReasonerException;
import cz.cvut.kbss.owl2query.owlapi.model.ABox;
import cz.cvut.kbss.owl2query.owlapi.model.ATermAppl;
import cz.cvut.kbss.owl2query.owlapi.model.DependencySet;
import cz.cvut.kbss.owl2query.owlapi.model.KnowledgeBase;
import cz.cvut.kbss.owl2query.owlapi.model.MultiQueryResults;
import cz.cvut.kbss.owl2query.owlapi.model.NotQueryAtom;
import cz.cvut.kbss.owl2query.owlapi.model.PelletOptions;
import cz.cvut.kbss.owl2query.owlapi.model.Query;
import cz.cvut.kbss.owl2query.owlapi.model.QueryAtom;
import cz.cvut.kbss.owl2query.owlapi.model.QueryAtomFactory;
import cz.cvut.kbss.owl2query.owlapi.model.QueryImpl;
...
import cz.cvut.kbss.owl2query.owlapi.parser.ARQParser;
import cz.cvut.kbss.owl2query.owlapi.parser.QueryParser;
import cz.cvut.kbss.owl2query.owlapi.util.ATermUtils;
import cz.cvut.kbss.owl2query.owlapi.util.Bool;
import cz.cvut.kbss.owl2query.owlapi.util.DisjointSet;
...
public class QueryEngine {
	...
	public static List<Query> split(Query query) {
		try {
			...
			final Collection<Set<ATermAppl>> equivalenceSets = disjointSet
					.getEquivalanceSets();
				...
				if (!ATermUtils.isVar(clazz)) {
					...
					if ((inferred != null) && !inferred.isEmpty()) {
						if (inferred.contains(clazz)) {
							...
							final Set<ATermAppl> subs = kb.getTaxonomy()
									.getFlattenedSubs(clazz, false);
							final Set<ATermAppl> eqs = kb
	...
	public static boolean execBooleanABoxQuery(final Query query) {
		...
		if (allTriplesSatisfied.isKnown()) {
			...
			if (!query.getConstants().isEmpty()) {
				final ATermAppl testInd = query.getConstants().iterator()
						.next();
	...
	public static boolean checkGround(final QueryAtom atom,
		...
		case DirectSubClassOf:
			for (final Set<ATermAppl> a : kb.getSubClasses(arguments.get(1),
					true)) {
		...
		case DirectSubPropertyOf:
			for (final Set<ATermAppl> a : kb.getSubProperties(arguments.get(1),
					true)) {
				if (a.contains(arguments.get(0))) {
					return true;
				}

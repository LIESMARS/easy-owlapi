...
public class QueryImpl implements Query {
	private static final ATermAppl DEFAULT_NAME = ATermUtils
			.makeTermAppl("query");
	 ...
	 * {@inheritDoc} TODO
	 */
	public ATermAppl rollUpTo(final ATermAppl var,
			final Collection<ATermAppl> stopList, final boolean stopOnConstants) {
		...
		List<ATermAppl> classParts = Collections.emptyList();
	...
	// TODO optimize - cache
	private List<ATermAppl> getClasses(final ATermAppl a) {
	 ...
	 * TODO
	 */
	private ATermAppl rollEdgeOut(final QueryPredicate allowed,
			final QueryAtom atom, final Set<ATermAppl> visited,
			final Collection<ATermAppl> stopList) {
		...
		case PropertyValue:
			...
			if (visited.contains(obj)) {
				List<ATermAppl> temp = getClasses(obj);
				if (temp.isEmpty()) {
			...
			List<ATermAppl> targetClasses = getClasses(obj);
			...
			if (targetClasses.isEmpty()) {
				if (targetOuts.isEmpty()) {
					...
					List<ATermAppl> outs = Collections.emptyList();
	...
	private ATermAppl rollEdgeIn(final QueryPredicate allowed,
			final QueryAtom atom, final Set<ATermAppl> visited,
			final Collection<ATermAppl> stopList) {
		switch (atom.getPredicate()) {
		case PropertyValue:
			...
			final ATermAppl pred = atom.getArguments().get(1);
			final ATermAppl obj = atom.getArguments().get(2);
			ATermAppl invPred = kb.getRBox().getRole(pred).getInverse()
					.getName();
			...
			if (visited.contains(subj)) {
				List<ATermAppl> temp = getClasses(subj);
				if (temp.isEmpty()) {
			...
			List<ATermAppl> targetClasses = getClasses(subj);
			...
			if (targetClasses.isEmpty()) {
				if (targetIns.isEmpty()) {
					...
					List<ATermAppl> ins = Collections.emptyList();
	...
	public void remove(QueryAtom atom) {
		...
		final Set<ATermAppl> toRemove = new HashSet<ATermAppl>(atom
				.getArguments());

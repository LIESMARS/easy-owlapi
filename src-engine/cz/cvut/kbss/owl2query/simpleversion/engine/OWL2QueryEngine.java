...
import cz.cvut.kbss.owl2query.simpleversion.model.Configuration;
...
import cz.cvut.kbss.owl2query.simpleversion.parser.arq.SparqlARQParser;
import cz.cvut.kbss.owl2query.simpleversion.util.DisjointSet;
...
public class OWL2QueryEngine {
	...
	public static <G> List<InternalQuery<G>> split(InternalQuery<G> query) {
		try {
			final Set<Variable<G>> resultVars = new HashSet<Variable<G>>(query
					.getResultVars());
	...
	private static <G> void simplify(InternalQuery<G> query) {
		...
		for (final QueryAtom<G> atom : new ArrayList<QueryAtom<G>>(query
				.getAtoms())) {
		...
		if (allTriplesSatisfied != null) {
			...
			if (!query.getConstants().isEmpty()) {
				final GroundTerm<G> testInd = query.getConstants().iterator()
						.next();
				final G testClass = query.rollUpTo(testInd, Collections
						.<Term<G>> emptySet());

				if (log.isLoggable(Level.FINER))
					log.finer("Boolean query: " + testInd + " -> " + testClass);

				querySatisfied = kb.isTypeOf(testClass, testInd
						.getWrappedObject(), false);
			} else {
				final Variable<G> testVar = query.getUndistVars().iterator()
						.next();
				final G testClass = query.rollUpTo(testVar, Collections
						.<Term<G>> emptySet());
	...
	public static <G> boolean checkGround(final QueryAtom<G> atom,
		...
		case EquivalentClass:
			return kb.getClassHierarchy().getEquivs(
					arguments.get(0).asGroundTerm().getWrappedObject())
					.contains(
							arguments.get(1).asGroundTerm().getWrappedObject());
		case SubClassOf:
			return kb.getClassHierarchy().getSubs(
					arguments.get(1).asGroundTerm().getWrappedObject(), false)
					.contains(
							arguments.get(0).asGroundTerm().getWrappedObject());
		case DirectSubClassOf:
			return kb.getClassHierarchy().getSubs(
					arguments.get(1).asGroundTerm().getWrappedObject(), true)
					.contains(
							arguments.get(0).asGroundTerm().getWrappedObject());
		case StrictSubClassOf:
			return kb.getClassHierarchy().getSubs(
					...
					.contains(
							arguments.get(0).asGroundTerm().getWrappedObject())
					&& !kb.getClassHierarchy().getEquivs(
							arguments.get(1).asGroundTerm().getWrappedObject())
							.contains(
									arguments.get(0).asGroundTerm()
											.getWrappedObject());
			// case DisjointWith:
			// return kb.isDisjoint(arguments.get(0).asGroundTerm(), arguments
			// .get(1).asGroundTerm());
			// case ComplementOf:
			// return kb.isComplement(arguments.get(0).asGroundTerm(), arguments
			// .get(1).asGroundTerm());
			// case EquivalentProperty:
			// return kb.isEquivalentProperty(arguments.get(0).asGroundTerm(),
			// arguments.get(1).asGroundTerm());
		case SubPropertyOf:
			return kb.getPropertyHierarchy().getSubs(
					arguments.get(1).asGroundTerm().getWrappedObject(), false)
					.contains(
							arguments.get(0).asGroundTerm().getWrappedObject());
		...
		case StrictSubPropertyOf:
			return kb.getPropertyHierarchy().getSubs(
					...
					.contains(
							arguments.get(0).asGroundTerm().getWrappedObject())
					&& !kb.getPropertyHierarchy().getEquivs(
							arguments.get(1).asGroundTerm().getWrappedObject())
							.contains(
									arguments.get(0).asGroundTerm()
											.getWrappedObject());
			...
			// 1).asGroundTerm());
		case ObjectProperty:
			return kb.is(arguments.get(0).asGroundTerm().getWrappedObject(), OWLObjectType.OWLObjectProperty);
		case DatatypeProperty:
			return kb.is(arguments.get(0).asGroundTerm().getWrappedObject(), OWLObjectType.OWLDataProperty);
			// case Functional:
			// return kb.isFunctionalProperty(arguments.get(0).asGroundTerm());
			// case InverseFunctional:
			// return kb.isInverseFunctionalProperty(arguments.get(0)
			// .asGroundTerm());
			// case Symmetric:
			// return kb.isSymmetricProperty(arguments.get(0).asGroundTerm());
			// case Transitive:

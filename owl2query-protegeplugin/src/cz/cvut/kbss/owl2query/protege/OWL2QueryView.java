...
import cz.cvut.kbss.owl2query.graph.model.IQueryGraphFactory;
...
import cz.cvut.kbss.owl2query.graph.util.TermSelection.SelectionPartner;
...
//                    sys.fireReasonerChanged();
		...
		final SelectionPartner partner = new TermSelection.StringSelectionPartner(sys.getSelection()) {
...
////							find uris and deside wether they are data or object properties
//							List<Term> props = ((CompoundEdge)val).getProperties();
//							for(Term prop : props){
//								if(prop.isGround()){
//									IRI iri = IRI.create(prop.asGroundTerm().getWrappedObject().toString());
...
//							if(t.isGround()){
//								IRI iri = IRI.create(t.asGroundTerm().getWrappedObject().toString());
//								OWLObject owlobject = null; 
    ...
    private JFileChooser getFileChooser() {
        if (fileChooser == null) {
            ...
            fileChooser.setFileFilter(new FileNameExtensionFilter(
                    ResourceLoader.getResource("file_description", this), "sparql"));
        }
        return fileChooser;
    }
...
//        try {
//			ontology = new OWLOntologyMerger(new OWLOntologyImportsClosureSetProvider(manager, ontology)).createMergedOntology(manager, IRI.create(ontology.getOntologyID().getOntologyIRI().toString()+ "_MERGED"));
...
//        });
    ...
    private void initComponents() {
        ...
        final JRadioButton rbtGraph = new JRadioButton(ResourceLoader.getResource("graph", this));
...
//                        txpQuery.setText(w.toString());
                        serializedLayouts = qgp.getLayoutStoreEditor().storeToString();
                        //layout the genereal tab
                        JSplitPane gen = qgp.getGeneralPanel();
...
//                    fc.setVisible(true);
                    ...
                    if (ret == JFileChooser.APPROVE_OPTION) {
                        ...
                        qgp.getLayoutStoreEditor().loadStoreFromFile(currentFile);
                        serializedLayouts = qgp.getLayoutStoreEditor().storeToString();
                        pnlResult.clearResults();
                    }
        ...
        openAction.putValue(Action.SHORT_DESCRIPTION, ResourceLoader.getResource("open_query", this));
